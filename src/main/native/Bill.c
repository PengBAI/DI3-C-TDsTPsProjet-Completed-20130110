...
void Bill_save(Document * document) {
    char buf[1024];
    sprintf(buf, "../bill-%s.dat", document->docNumber);
...
void Bill_load(const char * docNumber, Document * document) {
    char buf[1024];
    sprintf(buf, "../bill-%s.dat", docNumber);
...
static GtkTreeModel * Bill_loadModel(void) {
    ...
    if (directory == NULL) {
        ...
        errordialog = gtk_message_dialog_new(NULL, GTK_DIALOG_DESTROY_WITH_PARENT,
                GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE, "Impossible de lister les factures");
        ...
        while ((entry = g_dir_read_name(directory)) != NULL) {
            if (icaseStartWith("bill-", entry) && icaseEndWith(".dat", entry)) {
                char * filename = concatenateString("../", entry);
...
static int Bill_select(Document * document) {
    ...
    dialog = gtk_dialog_new_with_buttons("Choix d'une facture", NULL, GTK_DIALOG_MODAL
...
void Bill_new(GtkWidget * UNUSED( widget), GtkWindow * parent) {
    ...
    document.editDate = formatDate(tm->tm_mday, tm->tm_mon, 1900 + tm->tm_year);
