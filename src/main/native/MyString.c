...
int icaseCompareString(const char * str1, const char * str2) {
    while( toLowerChar(*str1) != '\0' && toLowerChar(*str2) != '\0')
    {
...
void copyStringWithLength(char * dest, const char * src, size_t destSize) {
    size_t min,i;
    if(stringLength(src) < destSize - 1)
        min = stringLength(src);
    else
        min = destSize - 1;
    for(i = 0; i < min; i++)
    {
...
char * duplicateString(const char * str) {
    ...
    if(newStr == NULL)
        fatalError("duplicateString() malloc is error!");
...
int icaseStartWith(const char * start, const char * str) {
    if(stringLength(start) > stringLength(str))
        return FALSE;
    while(*start != '\0')
    {
        if( toLowerChar(*start) == toLowerChar(*str))
        {
            start++;
            str++;
        }
...
/** Test if the string str ends by the string end, ignoring the case of the characters.
...
int icaseEndWith(const char * end, const char * str) {
    int endFinIndice = (int) (stringLength(end) - 1);
    int strFinIndice = (int) (stringLength(str) - 1);
    if(endFinIndice > strFinIndice)
        return FALSE;
    while(endFinIndice >= 0)
    {
        if( toLowerChar(end[endFinIndice]) == toLowerChar(str[strFinIndice]))
        {
            endFinIndice--;
            strFinIndice--;
        }
...
char * concatenateString(const char * str1, const char * str2) {
    ...
    size_t str2length = stringLength(str2);
    size_t i = 0;
    char * newstring;
    newstring = (char *)malloc(str1length + str2length + 1);
    if(newstring == NULL)
        fatalError("concatenateString() malloc is error!");
    while(str1[i] != '\0')
    {
        newstring[i] = str1[i];
        i++;
    }
    while(str2[i - str1length] != '\0')
    {
...
char * subString(const char * start, const char * end) {
    size_t i = 0;
    size_t startlength = stringLength(start);
    size_t endlength = stringLength(end);
    char *substr;
    substr = (char *) malloc(startlength - endlength + 1);
    if(substr == NULL)
        fatalError("subString() malloc is error!");
    while(i < (startlength - endlength))
    {
        substr[i] = start[i];
        i++;
    }
...
const char *indexOfString(const char *meule_de_foin, const char *aiguille) {
    size_t meulength = stringLength(meule_de_foin);
    size_t aiguillength = stringLength(aiguille);
    size_t i = 0;
    size_t j = 0;
    if(meulength < aiguillength)
        return NULL;
    while(meule_de_foin[i] != '\0')
    {
        if(meule_de_foin[i] == aiguille[j])
        {
            ...
            j++;
            if(aiguille[j] == '\0')
                return meule_de_foin + i - aiguillength ;
        }
        else
        {
            j = 0;
...
char * insertString(const char * src, int insertPosition, const char * toBeInserted,int insertLength) {
    ...
    size_t i, j, z;
    char * newString;
    newString = (char *)malloc(srclength + (size_t)insertLength + 1);
    if(newString == NULL)
        fatalError("insertString() malloc is error!");
    for(i = 0; i < (size_t)insertPosition; i++)
    {
        newString[i] = src[i];
    }
    for(j = 0; j < (size_t)insertLength; j++)
    {
        newString[i] = toBeInserted[j];
        i++;
    }
    for(z = (size_t)insertPosition; z < srclength; z++)
    {
        newString[i] = src[z];
        i++;
    }
