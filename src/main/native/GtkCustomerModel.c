...
void GtkCustomerModel_append_record(GtkCustomerModel * custom_list) {
    ...
    if (CustomerRecord_edit(&record)) {
        ...
        path = gtk_tree_path_new();
        gtk_tree_path_append_index(path, CustomerDB_getRecordCount(
                custom_list->clientDB) - 1);
...
void GtkCustomerModel_insert_record(GtkCustomerModel * custom_list,
        gint recordIndex) {
    GtkTreeIter iter;
    GtkTreePath *path;
    CustomerRecord record;
    ...
    CustomerRecord_init(&record);
    if (CustomerRecord_edit(&record)) {
        CustomerDB_insertRecord(custom_list->clientDB, recordIndex, &record);
...
void GtkCustomerModel_remove_record(GtkCustomerModel * custom_list,
        gint recordIndex) {
    if (CustomerDB_getRecordCount(custom_list->clientDB) > 0) {
        GtkTreePath *path;
